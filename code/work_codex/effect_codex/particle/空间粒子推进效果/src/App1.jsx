...
const particleFragmentShader = `
  ...
  void main() {
    ...
    // halo控制的是 每个粒子圆点从中心到边缘的透明度衰减形状，也就是粒子的“光晕/柔边”。
...
const contourEvents = [
  {
    type: 'sphere',
    // 首个轮廓提前进入显现阶段：ambientCount 为 0 时也能单独检查轮廓层是否渲染。
    start: 0.06,
    duration: 0.42,
    countRatio: 0.24,
...
function buildContourData(totalCount) {
  ...
  contourEvents.forEach((event, eventIndex) => {
    ...
    for (let localIndex = 0; localIndex < count && cursor < totalCount; localIndex += 1) {
      ...
      const color = pickParticleColor(seed);
      const densityBand = Math.sin(local.x * 4.2 + local.y * 2.8 + seededRandom(seed * 2.2) * 4.0);

      // 通过丢弃一部分弱密度粒子，制造几何轮廓有疏有密的自然断裂。
      const density = densityBand > -0.42 ? randomRange(seed * 3.3, 0.42, 1.0) : randomRange(seed * 3.7, 0.08, 0.28);
      ...
      0.38：少量大颗粒增强
      */
      visual[offset4] = randomRange(seed * 8.5, 0.54, 0.95) + Math.pow(seededRandom(seed * 9.8), 7.5) * 0.88;
