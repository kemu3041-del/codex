...
function exposeMorphState(fromIndex, toIndex, progress) {
...
function applyMorph(fromIndex, toIndex, progress) {
  exposeMorphState(fromIndex, toIndex, progress);
  window.dispatchEvent(new CustomEvent('particle-story-morph', {
    detail: { fromIndex, toIndex, progress }
  }));
}
...
function syncModelToScroll() {
  ...
  if (scrollPosition <= centers[0]) {
    ...
    applyMorph(modelIndex, modelIndex, 0);
    setProgress(0);
    return;
  }
  ...
  if (scrollPosition >= centers[lastIndex]) {
    ...
    applyMorph(modelIndex, modelIndex, 1);
  ...
  for (let index = 0; index < lastIndex; index += 1) {
    if (scrollPosition >= centers[index] && scrollPosition < centers[index + 1]) {
      ...
      applyMorph(fromModelIndex, toModelIndex, progress);
