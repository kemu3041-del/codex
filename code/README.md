...
├── README.md        # 工作区说明
├── js/              # 公共常用前端插件，本地预览和 SaaS/CMS 交付可复用
...
- 根目录 `js/` 放公共常用插件，例如 GSAP、ScrollTrigger、Lenis、Swiper；新建静态 Demo 优先从这里引用，避免 CDN 网络问题影响本地验证。
...
- 数据接入：先判断交付形态。标准 HTML/JS/GSAP 页面把标题、描述、图片、按钮、列表等内容写在 HTML 中；React/Vite 等工程化项目才整理成对象、接口数据、模拟数据和适配层。
- 复用方式：静态页面优先沉淀成可复制的 HTML 结构、CSS 类和效果配置；工程化项目再沉淀成组件、配置对象、Hooks 或可复用模块。
...
- ES6、现代 CSS 和成熟动画/交互库不是默认禁用项；平台和目标浏览器支持时可以直接使用。
- GSAP、ScrollTrigger、Three.js、Swiper、Lenis 等库可以通过 CDN、平台资源管理或本地静态资源引入，不需要为了“可粘贴交付”而强行手写所有能力。
- 本工作区优先使用根目录 `js/` 的公共本地插件做开发验证；交付到 SaaS/CMS 时，再按平台规则替换成平台资源地址、CDN 地址或打包后的静态文件。
- 子项目引用示例：`effect_codex/scroll-demo/index.html` 中使用 `../../js/gsap.min.js` 和 `../../js/ScrollTrigger.min.js`。
- 真正需要避免的是必须经过 Vite、Webpack、Babel、TypeScript 编译，或依赖本地 `node_modules` 才能运行的源码。
- 复杂滚动叙事、路径跟随、分阶段触发、滚动固定屏优先考虑 `GSAP + ScrollTrigger`，再根据平台规则决定 CDN 或打包方式。
- 数据组织规则：
  - 标准页面、普通 JS 效果、GSAP/ScrollTrigger 静态 Demo，内容直接放在 `index.html`，不要默认用 JS 数组对象动态生成页面主体内容。
  - React、Vite、Webpack 等需要构建的项目，才把内容做成 `cards`、`sections`、`products` 等对象或数组，方便后续替换为 SaaS/CMS 数据。
  - 动效 JS 应聚焦动画、交互和状态；静态项目如需字段映射，在 README 里说明 HTML 中对应位置即可。
